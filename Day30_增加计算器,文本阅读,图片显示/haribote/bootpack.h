...
void asm_inthandler0c(void);                                     // 负责处理栈异常中断
void asm_end_app(void);                                          // 返回cmd_app
// 用汇编封装的结束
...
int timer_cancel(struct TIMER *timer);                               // 取消指定计时器
void timer_cancelall(struct FIFO32 *fifo);                           // 取消应用程序结束时不需要的定时器
...
void console_task(struct SHEET *sheet, int memtotal);                          // 控制台任务
void cons_newline(struct CONSOLE *cons);                                       // 换行,当到达最后一行时还会自动滚动
void cons_putchar(struct CONSOLE *cons, int chr, char move);                   // 显示
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal); // 命令行
void cmd_mem(struct CONSOLE *cons, int memtotal);                              // mem命令
void cmd_cls(struct CONSOLE *cons);                                            // cls命令
void cmd_dir(struct CONSOLE *cons);                                            // dir命令
void cmd_exit(struct CONSOLE *cons, int *fat);                                 // exit命令
void cmd_start(struct CONSOLE *cons, char *cmdline, int memtotal);             // start命令(打开一个新命令行并执行指定程序)
void cmd_ncst(struct CONSOLE *cons, char *cmdline, int memtotal);              // nsct命令(不打开新命令行执行程序)
void cmd_langmode(struct CONSOLE *cons, char *cmdline);                        // langmode命令(语言模式切换，0,1)
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax); // API处理
void hrb_api_linewin(struct SHEET *sht, int x0, int y0, int x1, int y1, int col);     // 画线API处理
...
int *inthandler0c(int *esp);                                                          // 栈异常中断
