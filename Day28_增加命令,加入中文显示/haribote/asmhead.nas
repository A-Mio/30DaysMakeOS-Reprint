...
scrn320:
		...
		MOV		AH,0x00			; AH等于0时，AL可以选一些显示模式
		INT		0x10			; 视屏服务终端
		MOV		BYTE [VMODE],8	; 屏幕的模式（参考C语言的引用）
		MOV		WORD [SCRNX],320
		MOV		WORD [SCRNY],200
		MOV		DWORD [VRAM],0x000a0000	 ;每种模式有固定地址
...
[INSTRSET "i486p"]				; 想要使用486指令的叙述
		...
		OR		EAX,0x00000001	; 设bit0为1（保护模式过渡）
		MOV		CR0,EAX
		JMP		pipelineflush
pipelineflush:
...
skip:
		...
		JMP		DWORD 2*8:0x0000001b	; C语言开始位置
