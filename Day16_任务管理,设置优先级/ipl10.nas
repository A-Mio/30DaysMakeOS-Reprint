...
next:							; 到这就顺利的读完了一个扇区
		...
		MOV		DH,0
		ADD		CH,1
		CMP		CH,CYLS			; 读的柱面，CYLS在一开始定义的
		JB		readloop		; CH < CYLS 跳转到readloop

; 读取完毕，跳转到haribote.sys执行！
		MOV		[0x0ff0],CH		; 记下IPL读到了什么程度，存着，asmhead.nas中可能用到
		JMP		0xc200			; asmhead.nas中的程序装载位置
...
msg:
		DB		0x0a, 0x0a		; 换行两次
		DB		"Load Error" 	;BD 可直接写字符串，汇编会自动查找每个字符对应编码
		DB		0x0a			; 换行
		DB		0

		RESB	0x7dfe-$		; 填写0x00直到0x001fe $是一个变量，表示当前字节数
